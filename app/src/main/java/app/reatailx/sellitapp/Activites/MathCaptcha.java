...
public class MathCaptcha extends Captcha {
    ...
    protected Bitmap image() {
        ...
        LinearGradient gradient = new LinearGradient(0, 0, getWidth() / 4, this.height / 2, color(), color(), Shader.TileMode.MIRROR);
        Paint p = new Paint();
        p.setDither(true);
        p.setShader(gradient);
        Bitmap bitmap = Bitmap.createBitmap(getWidth(), this.height, Bitmap.Config.ARGB_8888);
        Canvas c = new Canvas(bitmap);
        c.drawRect(0, 0, 100, 80, p);

        LinearGradient fontGrad = new LinearGradient(0, 0, getWidth() / 4, this.height / 2, color(), color(), Shader.TileMode.CLAMP);
        Paint tp = new Paint();
        tp.setDither(true);
        tp.setShader(fontGrad);
        tp.setTextSize(getWidth() / this.height * 10);
        Random r = new Random(System.currentTimeMillis());
        one = r.nextInt(9) + 1;
        two = r.nextInt(9) + 1;
        math = r.nextInt((options == MathOptions.PLUS_MINUS_MULTIPLY)?3:2);
